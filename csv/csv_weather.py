...
def split_date_and_create_time_range(
	...
	tmp_column: str = "TMP",
	aa1_column: str = "AA1",
	observation_column: str = "obsercation",
) -> Path:
	"""
	...
	parsed_observation = pd.to_numeric(aa1_parts[0], errors="coerce")
	if observation_column in weather_df.columns:
		observation_values = pd.to_numeric(weather_df[observation_column], errors="coerce")
	else:
		observation_values = pd.Series(pd.NA, index=weather_df.index, dtype="Float64")
	observation_values.loc[has_aa1_comma_format] = parsed_observation.loc[has_aa1_comma_format]
	weather_df[observation_column] = observation_values
...
if __name__ == "__main__":
	...
	# ("split_date_time",)
	# ("remove_empty_aa1",)
	# ("keep_columns", "split_date_time", "remove_empty_aa1")
	STEPS_TO_RUN = ("keep_columns", "split_date_time", "remove_empty_aa1")
